...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        Configures Gemini API, selects sources, and initiates scraping.
        """
        try:
            ...
            self.stderr.write(self.style.ERROR(f"Config failed: {e}"))
            return

        if options['source']:
            sources = ScrapingSource.objects.filter(name__iexact=options['source'], is_active=True)
            if not sources:
                self.stderr.write(self.style.ERROR(f"No active source named '{options['source']}' found."))
                return
        else:
        ...
        if not news_list_page:
            self.stderr.write(self.style.ERROR("A 'NewsListPage' must be created in Wagtail first."))
            return

        for source in sources:
            self.stdout.write(self.style.NOTICE(f"Scraping source: {source.name}"))
            self.scrape_source(source, news_list_page)

        self.stdout.write(self.style.SUCCESS("All scraping tasks complete."))

    def scrape_source(self, source: ScrapingSource, news_list_page: NewsListPage):
        """
        Scrapes a single source, parses articles using Gemini, and saves them.
        """
        try:
            ...
            if not main_content:
                self.stderr.write(self.style.ERROR(f"Could not find selector '{source.html_selector}' on page."))
                return
            html_snippet = main_content.prettify()
        except requests.exceptions.RequestException as e:
            self.stderr.write(self.style.ERROR(f"URL fetch error: {e}"))
            return
        ...
        try:
            ...
            if not response.parts:
                self.stderr.write(self.style.ERROR("Gemini returned an empty response."))
                self.stderr.write(f"Reason: {response.prompt_feedback}")
                return
            cleaned_response = clean_json_response(response.text)
            if cleaned_response is None:
                self.stderr.write(self.style.ERROR("Could not find a valid JSON object in Gemini's response."))
                self.stderr.write(f"--- RAW RESPONSE ---\n{response.text}\n---")
                return
            parsed_data = json.loads(cleaned_response)
            validated_articles = ArticleList(**parsed_data)
        except Exception as e:
            self.stderr.write(self.style.ERROR(f"Gemini processing failed: {e}"))
            return
        ...
        for article_data in validated_articles.articles:
            try:
                ...
                existing_page.save()
                self.stdout.write(self.style.NOTICE(f"Updated: '{existing_page.title}'"))
                ...
                self.stdout.write(self.style.SUCCESS(f"Added: '{new_page.title}'"))